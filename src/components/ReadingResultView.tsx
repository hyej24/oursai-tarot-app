...
function generateDailyTemperatureReading(card: TarotCard | undefined): StandardReadingResult {
  ...
  const cardSpecificCautions = [
    `${cardName}의 흐름에서는 질문자님이 먼저 결론을 정해 놓고 상대 반응을 해석하는 걸 조심해야 해요.`,
    `${firstKeyword}이 강한 날이라, 작은 말투 하나를 너무 크게 받아들이면 마음이 먼저 피곤해질 수 있어요.`,
    `오늘은 상대의 속도가 질문자님 기대보다 느릴 수 있어요. 느리다는 이유만으로 마음이 없다고 단정하지 않는 게 좋아요.`,
    `${keywordText} 기운이 섞여 있어서, 서운함을 바로 꺼내면 대화가 설명보다 방어로 흘러갈 수 있어요.`,
    `오늘은 확인받고 싶은 마음이 커질수록 관계 온도가 오히려 예민해질 수 있어요.`,
    `${cardName}이 보여주는 흐름상, 상대를 시험하듯 던지는 말은 피하는 편이 좋아요.`,
    `마음이 급해질수록 표현이 딱딱해질 수 있어요. 오늘은 말의 온도를 먼저 살피는 게 중요해요.`,
    `${firstKeyword} 때문에 기대와 현실 사이에서 마음이 출렁일 수 있으니, 하루의 반응만으로 전체 관계를 판단하지 마세요.`
  ];
  const cardSpecificAdvices = [
    `${cardName}의 흐름을 살리려면 오늘은 가볍고 편한 말부터 시작해 보세요.`,
    `${firstKeyword}이 좋은 방향으로 흐르려면 질문보다 리액션을 먼저 주는 게 좋아요.`,
    `상대가 편하게 답할 수 있도록 짧은 안부나 자연스러운 농담처럼 부담 없는 접점을 만들어 보세요.`,
    `${keywordText} 기운을 부드럽게 쓰려면 질문자님이 먼저 여유 있는 태도를 보여 주는 게 좋아요.`,
    `오늘은 길게 설명하기보다 한 문장만 담백하게 남기는 쪽이 더 잘 닿아요.`,
    `${cardName}의 흐름상, 상대를 몰아가기보다 대화가 이어질 여지를 남겨 두는 게 좋아요.`,
    `상대 반응을 기다릴 때도 바로 의미를 붙이지 말고, 다음 대화로 이어질 작은 실마리를 보는 게 좋아요.`,
    `오늘은 질문자님이 먼저 분위기를 편하게 열어 주면 관계 온도가 조금 더 안정적으로 올라갈 수 있어요.`
  ];
