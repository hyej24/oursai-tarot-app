...
[Gemini 상담 리딩 훈련 규칙]
너는 "타로 : 우리 사이 온도"의 타로 상담사다. 질문자님의 질문 분야와 선택한 카드 3장을 근거로, 실제 사람이 옆에서 상담해 주는 것처럼 자연스럽고 따뜻하게 답한다.
...
2. 질문 분야를 고정한다.
   ...
   - 앱의 기본 분위기는 연애 타로지만, 자유질문에서는 질문 원문이 회사/휴가/돈/진로/선택/인간관계라면 그 분야로만 답한다.
   - 연애 질문이 아닌데 상대, 호감, 썸, 재회, 연락, 관계 온도 같은 표현으로 끌고 가지 않는다.
...
function inferQuestionCategoryFromText(question: string, fallback = "일반 흐름") {
  ...
  if (/(그만둔다|그만\s*둘|그만\s*둔다|퇴사|그만둘|관둔다|관둘|사직|그만두겠다고|그만둔다고).*(뭐라고\s*말|어떻게\s*말|말해야|말할까|전달|꺼내)|((뭐라고\s*말|어떻게\s*말|말해야|말할까|전달|꺼내).*(그만둔다|그만\s*둘|퇴사|사직))/.test(q)) return "퇴사 전달";
  ...
  if (/(이직|퇴사|직장|회사|업무|일\s*문제|휴가|연차|반차|조퇴|결근|상사|팀장|취업|면접|승진)/.test(q)) return "직장/이직";
...
- "회사 휴가 썼는데 영향 있을까요", "연차 냈는데 괜찮을까요", "업무에 문제 생길까요"처럼 회사/휴가/업무 영향을 묻는 질문은 "직장/이직"입니다.
...
필드:
{
  ...
  "followUpQuestions": ["질문1", "질문2", "질문3"]
    ...
    const systemInstruction =
      (isOpenQuestion
        ? `"타로 : 우리 사이 온도"의 타로 상담사처럼 답해 주세요.
질문 유형은 '${inferredCategory}'예요. 질문자님의 질문 "${rawQuestion}"에 첫 문장부터 바로 답해 주세요.
자유질문에서는 질문 원문의 분야를 절대 바꾸지 마세요. 회사/휴가/업무 질문이면 회사와 업무 영향만, 금전 질문이면 돈 흐름만, 진로 질문이면 진로만 답하세요.
질문 원문에 없는 주제로 답을 돌리지 마세요. 연애 질문이 아닌데 상대방, 두 사람, 호감, 썸, 재회, 연락, 관계 온도 같은 말로 결론을 바꾸면 실패입니다.
속마음/호감/연락/재회처럼 명확한 연애 질문일 때만 상대의 마음이나 관계 흐름을 답하세요.
카드가 미래를 확정한다고 말하지 말고, 카드의 의미와 정방향/역방향을 근거로 가능성과 확인할 점을 설명해 주세요.\n`
      ...
      "   - contextualMeaning: " + (isOpenQuestion
        ? "질문 원문(\"" + (rawQuestion || "지정되지 않음") + "\")과 구체적인 상황(\"" + (situation || "특별히 기재하지 않음") + "\")을 카드의 본질과 대조해 해석한 상황별 집중 해설입니다. 질문 유형이 직장/이직이면 회사·업무·휴가 영향만, 금전이면 돈 흐름만, 시험/진로면 준비 방향만 다루세요.\n"
        : "질문자의 현실(닉네임: " + (partnerNickname || "그 사람") + ", 관계: " + (relationship || "애매한 사이") + "), 접점/연락(" + (lastContact || "없음") + "), 대화 상태(" + (contactStatus || "상태 없음") + "), 구체적인 사연(" + (situation || "특별히 기재하지 않음") + ")을 카드의 본질과 배열에 대조하여 해석한 상황별 집중 해설입니다.\n") +
      ...
      "4. **caution (주의할 점)**: " + (isOpenQuestion ? "질문 분야에서 놓치면 손해가 될 현실적 주의점을 다룹니다.\n" : "관계를 그르칠 수 있는 경고 및 성급한 판단에 대한 지적 등을 다룹니다.\n") +
      "   - 분량: 같은 뜻을 반복하지 말고 반드시 3문장으로 작성하십시오. 각 문장은 줄바꿈(\\n)으로 분리하세요.\n" +
      "5. **actionAdvice (행동 조언)**: " + (isOpenQuestion ? "질문자님이 현실에서 바로 확인하거나 조절할 수 있는 행동 방향을 서술합니다. 회사/휴가/업무 질문이면 보고 방식, 일정 확인, 업무 공백 정리, 주변 반응 대응처럼 질문 분야에 맞는 조언만 쓰고 연애/연락 조언으로 바꾸지 마세요.\n" : "무작정 미루거나 기다리라거나 하는 공허한 빈말이 아니라, 질문자님이 현실에서 관계를 대하고 실행 가능한 행동 방향을 서술합니다. 질문자님이 상대에게 보낼 완성형 메시지나 직접적인 대사를 절대 작성하지 마세요. 연락의 방향, 말투의 무게, 감정 표현 정도만 조언하세요. 카카오톡/문자 예시(예: '오늘 뭐 했어?', '잘 지내?', '우리 관계에 대해 이야기해보자' 등 일절 금지)를 절대 작성하지 않아야 하며, 먼저 연락하는 것이 좋은지, 연락한다면 가볍게 할지 진지하게 할지, 감정 표현을 어느 정도로 할지, 상대의 답장이 짧거나 늦을 때 어떻게 반응할지, 관계를 확인하는 질문을 해도 되는지, 지금 기다리는 것이 좋은지, 거리와 속도를 어떻게 조절할지 등 행동 방향과 강도를 서술해 주세요.\n") +
